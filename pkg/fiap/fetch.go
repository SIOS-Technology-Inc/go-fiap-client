...
type Fetcher interface {
	...
	FetchFetchchOldest(fromDate *time.Time, untilDate *time.Time, ids ...string) (pointSets map[string](model.ProcessedPointSet), points map[string]([]model.Value), fiapErr *model.Error, err error)
...
この関数は、FetchOnceメソッドを繰り返し呼び出し、取得したデータを結果のmapに追加します。
...
- fiapErr: fiap通信の<error>タグを格納する構造体。タグがない場合はnil
	- fiapErrの発生条件
		- processQueryRSでエラーが発生した場合
			- queryRS.Transport.Header.Errorがnilでない場合
				- queryRS.Transport.Header.Errorは、SOAP通信は成功したが、FIAP通信が失敗した場合に格納されるため、fiapErrとして返す
- err: goのエラー情報を格納する構造体。エラーが発生した場合、スタックトレースを含むエラー情報が返される。エラーがない場合はnil。
	- errの発生条件
		- fiapFetchでエラーが発生した場合
			- レシーバfで設定したconnectionURLが http:// または https:// で始まっていない場合
			- メソッドの引数のkeysの長さが0の場合
			- メソッドの引数のkeys.IDが空の場合
			- soap通信を行うclient.Callメソッドでエラーが発生した場合
		- processQueryRSでエラーが発生した場合
			- queryRS.Transportがnilの場合
				- データが取得できていないため、エラーとするとともに、エラーの原因を特定するためにhttp status codeを表示する
			- queryRS.Transport.Headerがnilの場合
				- SOAP通信に成功した場合はHeader内にokまたはerrorが格納されるため、Headerがnilの場合はエラーとする
				- エラーの原因を特定するためにhttp status codeを表示する
			- queryRS.Transport.Header.OKがnilでなく、queryRS.Transport.Bodyがnilの場合
				- SOAP通信に成功した場合はBody内にデータが格納されるため、Bodyがnilの場合はエラーとする
				- エラーの原因を特定するためにhttp status codeを表示する
*/
...
この関数は、Fetchメソッドを使用して、指定されたキーとIDのセットを使用してデータを取得します。
...
この関数は、FetchByIdWithKeyを使用して、指定された日付範囲とIDセット内のデータを取得します。
